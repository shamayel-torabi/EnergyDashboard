...
export class Color {
  ...
  */
  ...
  toBlackWhite(threshold: number): Color {
    var source = this.getSource();
    var average: string = (source[0] * 0.3 + source[1] * 0.59 + source[2] * 0.11).toFixed(0);
    var currentAlpha: number = source[3];

    threshold = threshold || 127;

    var a = (Number(average) < threshold) ? 0 : 255;
    this.setSource([a, a, a, currentAlpha]);
    return this;
  }
  /**
...
*/
  private _tryParsingColor(color: string): void {
    ...
    else if (color.match(Color.reRGBa)) {
      source = Color.sourceFromRgb(color);
    }
    else if (color.match(Color.reHSLa)) {
  ...
  private _rgbToHsl(r: number, g: number, b: number) {
    ...
    let h:number = 0, s:number = 0, l:number = 0,
      max:number = Util.max([r, g, b], null),
      min:number = Util.min([r, g, b], null);
  ...
  static reRGBa = /^rgba?\(\s*(\d{1,3}(?:\.\d+)?\%?)\s*,\s*(\d{1,3}(?:\.\d+)?\%?)\s*,\s*(\d{1,3}(?:\.\d+)?\%?)\s*(?:\s*,\s*((?:\d*\.?\d+)?)\s*)?\)$/;
   ...
   * @memberOf fabric.Color
   */
  static reHSLa = /^hsla?\(\s*(\d{1,3})\s*,\s*(\d{1,3}\%)\s*,\s*(\d{1,3}\%)\s*(?:\s*,\s*(\d+(?:\.\d+)?)\s*)?\)$/;
   ...
   * @memberOf fabric.Color
   */
  static reHex = /^#?([0-9a-f]{8}|[0-9a-f]{6}|[0-9a-f]{4}|[0-9a-f]{3})$/i;
  ...
  static sourceFromRgb(color: string): number[] {
    let match = color.match(Color.reRGBa);
    let r:number = 0, g:number = 0, b:number = 0, a:number = 0;
    if (match) {
      r = parseInt(match[1], 10) / (/%$/.test(match[1]) ? 100 : 1) * (/%$/.test(match[1]) ? 255 : 1);
  ...
  static sourceFromHsl(color: string): number[] {
    let match = color.match(Color.reHSLa);
    let h:number = 0, s:number = 0, l:number = 0, r:number = 0, g:number = 0, b:number = 0, a:number = 0;
    if (match) {

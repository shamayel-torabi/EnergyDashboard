...
public sealed class DbContextHealthCheck<TContext> : IHealthCheck where TContext : DbContext
{
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        ...
        {
            return HealthCheckResult.Healthy("اتصال به بانک اطلاعاتی با موفقیت برقرار شد.");
        }

        return HealthCheckResult.Unhealthy("اتصال به بانک اطلاعاتی با مشکل مواجه شده است. اتصال به بانک اطلاعاتی را بررسی کنید.");
    }
}

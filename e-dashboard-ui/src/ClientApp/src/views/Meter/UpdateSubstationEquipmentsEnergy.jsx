...
const UpdateSubstationEquipmentsEnergy = (props) => {
    const now = Date.now();
    ...
    const updateSubstationEquipmentsEnergy = async () => {
        const { startDate, endDate, updateRecords, stationId } = model;
        ...
        const url = `${apiUrl}/EnergyProfiles/UpdateSubstationEquipmentsEnergy/${stationId}/${startDate.toJSON()}/${endDate.toJSON()}/${updateRecords}`;
        ...
        try {
            ...
            if (response.ok) {
                ...
                const err = await response.json();
                console.error(err);
                toast.showToast("error", "خطا در به روز رسانی جدول پروفایل انرژی ایستگاه.");
                setResult('')
            }
        }
        catch (e) {
            toast.showToast("error", "خطا اتصال به سرور در به روز رسانی جدول پروفایل انرژی ایستگاه.");
            console.error(e);
            setResult('')
        }
        setLoading(false);
    ...
    const handleSubmit = async (event) => {
        ...
        await updateSubstationEquipmentsEnergy();
    }
    ...
    return (
        <CRow className="animated fadeIn">
            <CCol lg="6">
                <CCard>
                    <CCardHeader>
                        <strong>بروز رسانی انرژی ایستگاه</strong>
                    </CCardHeader>
...
export default UpdateSubstationEquipmentsEnergy;
